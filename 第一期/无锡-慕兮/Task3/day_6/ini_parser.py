...
class LQINIParser:
    ...
    # 返回float类型
    def get_folat(self, section, option):
        if self.ini:
    ...
    def add_section(self, section):
        if self.ini:
            return self.ini.add_section(section)
...
if __name__ == "__main__":
    ...
    ini.set_option(section, "host", "127.0.0.1")
    ini.set_option(section, "port", "3306")
    ini.set_option(section, "db", "autotesting")
    ini.set_option(section, "user", "root")
    ini.set_option(section, "password", "123456")
    ...
    port = ini.get_string("mysql", "port")
    ...
    # 修改mysql port的值为4000
    ini.set_option(section, "port", "4000")
